...
class AdamScaleOp : public framework::OperatorWithKernel {
 public:
...
class AdamScaleFunctor;

template <typename T>
class AdamScaleFunctor<T, GPUAdamScale> {
 private:
  T beta1_;
  T beta2_;
  T epsilon_;
 ...
 public:
  AdamScaleFunctor(T beta1, T beta2, T epsilon, const T* beta1_pow,
      ...
      : beta1_(beta1),
        ...
        new_param_scale_(new_param_scale) {}
  ...
  inline HOSTDEVICE void operator()(size_t i) const {
    ...
    adam_ratio =
        (adam_ratio < new_param_scale_) ? adam_ratio : new_param_scale_;
...
template <typename T>
class AdamScaleFunctor<T, CPUAdamScale> {
 private:
  T beta1_;
  T beta2_;
  T epsilon_;
...
template <typename T, typename Flavour>
class SparseAdamScaleFunctor;

template <typename T>
class SparseAdamScaleFunctor<T, GPUAdamScale> {
 private:
  T beta1_;
  T beta2_;
  T epsilon_;
 ...
 public:
  SparseAdamScaleFunctor(T beta1, T beta2, T epsilon, const T* beta1_pow,
                    const T* beta2_pow, const T* mom1, T* mom1_out,
                    const T* mom2, T* mom2_out, const T* lr, const T* grad,
                    const T* param, T* param_out, T new_param_scale,
                    const int64_t* rows, int64_t row_numel,
                    int64_t row_count, bool lazy_mode)
      : beta1_(beta1),
        beta2_(beta2),
        epsilon_(epsilon),
        beta1_pow_(beta1_pow),
...
template <typename T>
class SparseAdamScaleFunctor<T, CPUAdamScale> {
 private:
  T beta1_;
  T beta2_;
  T epsilon_;
    ...
    for (int64_t i = 0, j = 0; i != row_count; ++i) {
      if (i == *(rows_ + j)) {
        ...
        for (int64_t k = 0; k != row_numel_; ++k) {
          ...
          p -= adam_ratio * lr * (mom1 / (sqrt(mom2) + epsilon_));
          // Write back to global memory
...
class AdamScaleOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (grad_var->IsType<framework::LoDTensor>()) {
      ...
      AdamScaleFunctor<T, CPUAdamScale> functor(
      ...
      SparseAdamScaleFunctor<T, CPUAdamScale> functor(
          beta1, beta2, epsilon, beta1_pow->data<T>(), beta2_pow->data<T>(),
          mom1->data<T>(), mom1_out->mutable_data<T>(ctx.GetPlace()),
          mom2->data<T>(), mom2_out->mutable_data<T>(ctx.GetPlace()),
