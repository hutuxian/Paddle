...
void AdamScaleOp::InferShape(framework::InferShapeContext *ctx) const {
  PADDLE_ENFORCE_EQ(
      ctx->HasInput("Param"), true,
      platform::errors::NotFound(
                        "Input(Param) of AdamScaleOp should not be null."));
  PADDLE_ENFORCE_EQ(
      ctx->HasInput("Grad"), true,
      platform::errors::NotFound(
                    "Input(Grad) of AdamScaleOp should not be null."));
  PADDLE_ENFORCE_EQ(ctx->HasInput("Moment1"), true,
                    platform::errors::NotFound(
                    "Input(Moment1) of AdamScaleOp should not be null."));
  PADDLE_ENFORCE_EQ(ctx->HasInput("Moment2"), true,
                    platform::errors::NotFound(
                        "Input(Moment2) of AdamScaleOp should not be null."));
  PADDLE_ENFORCE_EQ(ctx->HasInput("LearningRate"), true,
                    platform::errors::NotFound(
                    "Input(LearningRate) of AdamScaleOp should not be null."));
  PADDLE_ENFORCE_EQ(ctx->HasInput("Beta1Pow"), true,
                    platform::errors::NotFound(
                    "Input(Beta1Pow) of AdamScaleOp should not be null."));
  PADDLE_ENFORCE_EQ(ctx->HasInput("Beta2Pow"), true,
                    platform::errors::NotFound(
                    "Input(Beta2Pow) of AdamScaleOp should not be null."));

  PADDLE_ENFORCE_EQ(ctx->HasOutput("ParamOut"), true,
                    platform::errors::NotFound(
                    "Output(ParamOut) of AdamScaleOp should not be null."));
  PADDLE_ENFORCE_EQ(ctx->HasOutput("Moment1Out"), true,
                    platform::errors::NotFound(
                    "Output(Moment1Out) of AdamScaleOp should not be null."));
  PADDLE_ENFORCE_EQ(ctx->HasOutput("Moment2Out"), true,
                    platform::errors::NotFound(
                    "Output(Moment2Out) of AdamScaleOp should not be null."));
  ...
  if (ctx->GetInputsVarType("Grad")[0] ==
    ...
    PADDLE_ENFORCE_EQ(
        ...
        platform::errors::InvalidArgument(
            "Param and Grad input of AdamScaleOp should have same dimension."
            "But received Param dims: [%s], Grad dims: [%s].",
            param_dims, ctx->GetInputDim("Grad")));
  }
  PADDLE_ENFORCE_EQ(
      param_dims, ctx->GetInputDim("Moment1"),
      platform::errors::InvalidArgument(
          "Param and Moment1 input of AdamScaleOp should have same dimension."
          "But received Param dims: [%s], Moment1 dims: [%s].",
          param_dims, ctx->GetInputDim("Moment1")));
  PADDLE_ENFORCE_EQ(
      param_dims, ctx->GetInputDim("Moment2"),
      platform::errors::InvalidArgument(
          "Param and Moment2 input of AdamScaleOp should have same dimension."
          "But received Param dims: [%s], Moment2 dims: [%s].",
          param_dims, ctx->GetInputDim("Moment2")));
...
framework::OpKernelType AdamScaleOp::GetExpectedKernelType(
    const framework::ExecutionContext &ctx) const {
  auto input_data_type = OperatorWithKernel::IndicateVarDataType(ctx, "Param");
  return framework::OpKernelType(input_data_type, ctx.GetPlace());
}

framework::OpKernelType AdamScaleOp::GetKernelTypeForVar(
...
class AdamScaleOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    ...
    AddInput("Beta1Tensor",
             "(Tensor<float32>, optional) If provided, AdamScale will use this "
    ...
    AddInput("NewParamScaleTensor",
             "(Tensor<float32>, optional) If provided, AdamScale will use this "
             "as new_param_scale, this has a higher priority than attr"
             " (new_param_scale), the shape of this tensor MUST BE [1].")
        .AsDispensable();
    ...
    AddAttr<float>("epsilon",
        ...
        .SetDefault(1.0e-8f);
    AddAttr<float>("new_param_scale",
                   "(float, default 5.0) "
                   "Constant for new param scale")
        .SetDefault(5.0f);
    AddAttr<bool>(
        "lazy_mode",
        "(bool, default false) "
...
REGISTER_OP_WITHOUT_GRADIENT(
    adam_scale, ops::AdamScaleOp, ops::AdamScaleOpMaker);
REGISTER_OP_CPU_KERNEL(
    adam_scale,
    ops::AdamScaleOpKernel<paddle::platform::CPUDeviceContext, float>,
    ops::AdamScaleOpKernel<paddle::platform::CPUDeviceContext, double>);
